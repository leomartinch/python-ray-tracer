...
def render_chunk(start_row, end_row, width, height, pixel_size, start_x, start_y, scene, progress_queue, camera_data):
    rows = []
    ...
    for h in range(start_row, end_row): # row
        y_pos = multiply_scalar(row_vector, ((pixel_size * h) + center_pixel))
        row = []

        for w in range(width): # column
            ...
            ir = int(255 * min(color[0], 1))
            ig = int(255 * min(color[1], 1))
            ib = int(255 * min(color[2], 1))

            row.append(f"{ir} {ig} {ib}")
        
        rows.append(row)
        progress_queue.put(1) # update progress for each completed row
    return rows
...
def get_object_hit(ray_origin, ray_direction, scene):   
    ...
    for object_id in scene:
        ...
        if not ray_intersects_box(ray_origin, ray_direction, object.bounding_box):
            continue
...
def ray_intersects_box(ray_origin, ray_direction, bounding_box): 
...
def render_image(image_width, image_height, image_resolution, samples, max_bounces, yaw, pitch, camera_location, field_of_view, filename, scene):
    ...
    num_processes = max(multiprocessing.cpu_count() - 6, 1) # weil ich nicht meinen ganzen pc freezen möchte
    chunk_size = height // num_processes
    chunks = [(i * chunk_size, (i + 1) * chunk_size if i != num_processes - 1 else height,
               width, height, pixel_size, start_x, start_y, scene)
    ...
    progress_process = multiprocessing.Process(target=progress_monitor, args=(progress_queue, total_rows))
    progress_process.start()

    # render chunks in parallel
    with multiprocessing.Pool(processes=num_processes) as pool:
        results = pool.starmap(render_chunk, [(start_row, end_row, width, height, pixel_size, start_x, start_y, scene, progress_queue, camera_data)
                                              for (start_row, end_row, width, height, pixel_size, start_x, start_y, scene) in chunks])

    progress_process.join()  # wait for progress bar process to finish

    # combine results into the final image
...
def get_camera_data(image_width, image_height, yaw, pitch, camera_location, field_of_view, samples, max_bounces):
    camera_data = {}
    # yaw und pitch nicht fertig geschafft
...
def transform_mesh(vertices, translation, rotation, scale):
    ...
    if rotation:
        ...
        for vertex in vertices:
            ...
            # keine scale für individuelle axen
...
image_filename = "ray-bounce-test-35"
...
if __name__ == "__main__": # für windows braucht man das für das multiprocessing
